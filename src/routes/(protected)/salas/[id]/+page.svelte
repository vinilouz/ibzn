...
<div class="mx-auto w-full max-w-7xl p-8">
	...
	<Card class="mb-8 overflow-hidden">
		{#if data.room.imageUrl}
			<div class="flex h-[500px] w-full items-center justify-center overflow-hidden bg-muted">
				<img
		...
		{:else}
			<div
				class="flex h-[500px] w-full flex-col items-center justify-center bg-gradient-to-br from-muted/50 to-muted"
			>
				<ImageOff class="h-24 w-24 text-muted-foreground" />
				<p class="mt-4 text-muted-foreground">Nenhuma imagem disponível</p>
			</div>
		{/if}
		...
		<CardHeader>
			<div class="flex items-start justify-between">
				<div class="flex-1">
					<CardTitle class="mb-2 text-3xl">{data.room.name}</CardTitle>
					<CardDescription class="text-lg">Sala #{data.room.number}</CardDescription>
				</div>
				<span
					class="inline-flex items-center rounded-full px-3 py-1 text-sm font-semibold {data.room
						.status
	...
	<Card class="mb-8">
		...
		<CardContent class="space-y-6">
			...
			<Button onclick={() => goto(`/salas?view=edit&id=${data.room.id}`)}>Editar Sala</Button>
		</CardContent>
	</Card>
						...
						<TableBody>
							{#each data.courses as course}
								<TableRow>
									...
									<TableCell>{course.facilitatorName || '-'}</TableCell>
									<TableCell>
									...
									<TableCell class="text-right">
										<Button
											size="sm"
											variant="outline"
											onclick={() => goto(`/cursos?view=edit&id=${course.id}`)}
										>
											Ver Curso
										</Button>
									</TableCell>
									...
									<TableCell class="text-right">
										<div class="flex justify-end gap-2">
											<Button
												...
												onclick={() =>
													goto(
														`/presenca?courseId=${item.courseId}&participantId=${item.participant.id}`
													)}
											>
		...
		<Card>
			<CardContent class="p-12 text-center">
				...
				<p class="mb-4 text-muted-foreground">
					Nenhum participante matriculado em cursos desta sala
				</p>
			</CardContent>
		</Card>
	{/if}
